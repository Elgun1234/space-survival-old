...
cur.execute("CREATE TABLE IF NOT EXISTS Information(Username,playtime,settings,highest_score,total_score,runs,bullets_shot)")



def check_user_pass(received_data):
    cur.execute("SELECT Username FROM Accounts WHERE Username=? AND Password=?", (received_data[1], hashlib.sha512(
        received_data[2].encode(encoding='UTF-8',
                                errors='strict')).digest()))  # check if pass and user of same entity matches recirved data
...
def add_data(received_data):
    cur.execute("INSERT INTO Accounts (Username, Password) VALUES (?,?)", (received_data[1], hashlib.sha512(
...
def fetch_hours(received_data):
    cur.execute("SELECT playtime,total_score,runs,bullets_shot ,highest_score FROM Information WHERE Username=?",
                (received_data[1],))
...
def update_info(received_data):  ## datetimme
    ...
    if info[4] < received_data[3]:
        cur.execute(
            "UPDATE Information SET playtime = ?,total_score= ?,runs= ?,bullets_shot= ?,highest_score= ? WHERE Username = ?",
            (str(new_time), new_total, runs, new_bullet, received_data[3], received_data[1]))
    else:

        cur.execute("UPDATE Information SET playtime = ?,total_score= ?,runs= ?,bullets_shot= ? WHERE Username = ?",
                    (str(new_time), new_total, info[2], new_bullet, received_data[1]))
...
while True:
    ...
    if received_data[0] == "leaderboard":
        print("leaderboard")
        print(leaderboard_fetch())

        data = pickle.dumps([leaderboard_fetch()])
        sock.send(data)
        sock.close()
